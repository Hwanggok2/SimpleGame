...
| DA-004 | Excel→Generated SO 가져오기 | `Planning/GameData.xlsx` 저장 후 `SimpleGame > Data > Import Excel` 실행 | `EnemyBalance`, `StageSpawn`, `PlayerLevelExp`, `AccountLevelExp`, `GlobalBalance` 시트 | 열 이름·숫자 범위·ID 참조·중복·레벨 연속성·Unity Prefab 및 활성 Scene SpawnPoint 참조를 전부 검증한 뒤 `Assets/Game/Data/Generated` 에셋을 일괄 갱신 | 검증된 데이터가 기존 Manifest 참조를 통해 다음 실행부터 즉시 반영 | `.xlsx`만 지원하고 수식 셀은 허용하지 않음. 오류가 하나라도 있으면 기존 정상 SO를 변경하지 않으며 오류 메시지에 시트·행·열을 표시 | Editor 완료/실패 Dialog와 Console 로그 | `GameDataExcelImporter`, `OpenXmlWorkbookReader`, `GameDataAssetBuilder` | 정상 파일은 결정적으로 같은 SO를 만들고 오류 파일은 기존 SO를 보존한 채 시트·행·열을 포함해 실패해야 함 | P1/구현 |
