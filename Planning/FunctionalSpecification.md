...
| DA-004 | Excel→Generated SO 가져오기 | `Planning/GameData_10min_Balance.xlsx` 저장 후 `SimpleGame > 데이터 > 엑셀 불러오기` 실행 | `GameString`을 포함한 8개 필수 시트와 선택 `StageSpawnEasy` 시트 | 열 이름·숫자 범위·ID 참조·난이도별 중복·레벨 연속성·Unity Prefab 및 활성 Scene SpawnPoint 참조를 전부 검증한 뒤 `Assets/Game/Data/Generated` 에셋을 일괄 갱신한다. `StageSpawn`은 보통, `StageSpawnEasy`는 쉬움으로 태깅하고 `GameString`은 `GameStringTable.asset`로 변환한다. | 검증된 게임 데이터와 문구가 기존 Manifest 참조를 통해 다음 실행부터 즉시 반영 | 기존 워크북 호환을 위해 `StageSpawnEasy`는 선택 시트다. `.xlsx`만 지원하고 수식 셀은 허용하지 않으며, 오류가 하나라도 있으면 기존 정상 SO를 변경하지 않음 | Editor 완료/실패 Dialog와 Console 로그 | `GameDataExcelImporter`, `OpenXmlWorkbookReader`, `GameDataAssetBuilder` | 정상 파일은 두 난이도와 GameString을 Generated SO에 반영하고 오류 파일은 기존 SO를 보존해야 함 | P0/구현 |
...
| IN-015 | 모드 1 교전 반경 원주 이동 | 모드 1 실제 방향 이동 중 경로상 첫 대상 또는 잠긴 생존 대상의 교전 반경 진입 | 대상이 아직 공격되지 않았거나 이동 관통 불가 또는 현재 PC-006 예산 없음 | 일반 Melee/Ranged는 `R_safe=max(0, enemyAttackRange-min(0.02, 0.1×enemyAttackRange))`, `R_engage=min(playerAttackRange,R_safe)`를 사용한다. Shield/Boss, 공격 사거리 0 이하 또는 Definition 없음은 Player 공격 사거리를 사용한다. 다음 위치가 교전 반경 안쪽이면 입력의 안쪽 법선 성분만 제거하고 접선 성분을 적용하며 정확한 안쪽 입력은 경계에서 정지한다. | 양측이 공격 가능한 반경을 유지하며 좌우로 선회하고, 경로 대상을 공격 전에 통과하지 않음 | 대상 사망·세대 변경 시 제약 해제. 바깥 방향 입력은 수정하지 않고 위치를 순간 이동시키지 않는다. 실제 방향 입력이 없으면 이 수동 이동 규칙 대신 자동 접근을 허용 | 원형 선회 이동 | `PlayerController`, `PlayerMovement` | Melee/Ranged는 Player/Enemy 사거리 중 안전한 작은 값, Shield/Boss는 Player 사거리를 써야 한다. 경로 우선 대상의 공격 전 통과 금지, 정면 정지, 대각 접선 이동, 바깥 입력 불변을 검증해야 함 | P0/확정 |
...
| PR-020 | 정전기·오물 투척 융합 | 융합 전용 오물 재사용·장판 틱 | 정전기·오물 마스터 snapshot | 기본 오물과 별도 마스터 스케줄러로 5개 구체를 4초마다 던진다. 각 장판은 Enemy와 SpawnGeneration을 기록해 생성 개체의 첫 피해 틱에만 마스터 정전기를 발동한다. | 장판별·생성 개체별 최초 1회 정전기 | 같은 장판 재진입은 재발동하지 않고, 다른 장판은 각각 발동한다. 풀 재사용으로 세대가 바뀌면 새 개체로 한 번 발동한다. 기본 오물 stream과 독립 | 오물 장판과 정전기 Arc | `PlayerCombatAbilities`, `FilthProjectile`, `EnemyWorldService` | 6틱 피해 유지, 같은 장판 같은 세대 1회, 겹친 장판 각각 1회, 세대 변경 후 재발동을 검증해야 함 | P0/확정 |
