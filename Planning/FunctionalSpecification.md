...
| PR-009 | 절단 카드 | `SEVER_TRAIL` 선택 | 관통 카드 1레벨 이상, Player 레벨 3 이상, 절단 미보유 | 실제 관통 시작 위치 `p₀`를 저장하고 0.15초 뒤 Player 현재 위치 `P(t+0.15)`까지 Player 프리팹의 `cutting` 형태로 검은 선분을 생성한다. 선분과 겹친 모든 Enemy에게 `2×A_side` 피해를 준다. | 일반 공격 피해와 절단 피해가 함께 적용 | 실제 관통당 최대 1개, 재사용 0.1초. 쿨타임 중 요청은 버림. 최대 1레벨, 가중치 45 | 완성된 검은 선분이 즉시 나타나 0.1초간 선형 페이드 | `PlayerCombatAbilities`, `SlashTrailEffect`, `CombatGeometry` | 같은 관통에서 절단이 2개 생성되지 않고, 0.15초 뒤 선분의 모든 겹친 Enemy가 2배 피해를 받아야 함 | P0/구현 |
...
| FX-008 | 절단·이동 참격 | PR-009 또는 PR-012 발동 | 시작·끝 위치 또는 이동 방향 | 절단은 실제 관통 시작점부터 0.15초 뒤의 현재 Player 위치까지 `cutting` 형태의 검은 선을 즉시 표시하고 선형 페이드하며, 이동 참격은 이동 방향에 수직인 선을 이동시킨다. | 범위와 진행 방향을 시각적으로 전달 | 절단과 이동 참격은 서로 독립된 효과이며 같은 Enemy에 각각 피해 가능 | 검은 절단선과 투사체 참격 | `SlashTrailEffect`, `MovingSlashProjectile` | 절단 선분과 이동 참격 충돌 범위가 화면 표시와 일치해야 함 | P1/구현 |
...
- 절단: 실제 관통 후 `t+0.15`에 선분 `[p₀, P(t+0.15)]` 생성, 재사용 0.1초, 겹친 Enemy 피해 `2A_side`
